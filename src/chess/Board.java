...
public class Board {
    ...
    public void init() {
        ...
        for (PlayingPiece[] playingPieces : board) {
            for (PlayingPiece playingPiece : playingPieces) {
                Chess.display.addGraphic(playingPiece.getTexture());
            }
        }
    ...
    public void move(int currX, int currY, int nextX, int nextY) {
        if (board[currY][currX].getColor() == (whiteMove ? 'w' : 'b')) {
            if (board[currY][currX].isLegalMove(currX, currY, nextY, nextX)) {
                moveFigureInBoardLogic(currX, currY, nextX, nextY);
                if (isKingUnderThreat(getKing().getX() / PlayingPiece.HEIGHT, getKing().getY() / PlayingPiece.WIDTH, getKing())) {
                    moveFigureInBoardLogic(nextX, nextY, currX, currY);
                    System.out.println("the king is under check");
                }
    ...
    private King getKing() {
        return whiteMove ? (King) board[whiteKingCords.y][whiteKingCords.x] : (King) board[blackKingCords.y][blackKingCords.x];
    }

    private void moveFigureInBoardLogic(int currX, int currY, int nextX, int nextY) {
